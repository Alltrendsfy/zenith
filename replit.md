...
*   **Backup System**: Complete data backup functionality for disaster recovery. Access restricted to admin and gerente roles, enforced at both backend (requireManager middleware on all /api/backup/* endpoints) and frontend (permission-based UI gating). Features include: JSON export of all database tables, backup history tracking with metadata (filename, file size, tables included, records count, notes), download-to-device for Google Drive storage, and daily reminder alert on dashboard at 17:30 Brasilia time if no backup was performed that day. The BackupAlert component uses timezone-aware logic to determine visibility and auto-dismisses after backup completion. Users without admin/gerente role see "Acesso Restrito" message when attempting to access backup page.
*   **Cost Center Filter in Financial Modules**: Accounts Payable and Accounts Receivable pages now include a "Centro de Custo" filter dropdown. Default option "Todos os Centros" shows all authorized records. When a specific cost center is selected, the queryKey includes `costCenterId` parameter which the queryClient automatically converts to a query string. Backend validates access permissions via `canAccessCostCenter` function before returning filtered results. Admin/Gerente roles can filter by any cost center; restricted roles can only filter by their assigned centers. Filter uses consistent pattern across all modules (Bank Statement, Accounts Payable, Accounts Receivable).
